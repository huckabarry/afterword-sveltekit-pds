...
import { reblogObject } from '$lib/server/mastodon-state';
...
export async function POST(event) {
	...
	if (!isLocalAfterwordObject(origin, objectId)) {
		...
		if (!targetActorId) throw error(400, 'Target object does not declare an actor');

		const remoteActor = await fetchRemoteActor(targetActorId);
		const inboxUrl = remoteActor.sharedInboxUrl || remoteActor.inboxUrl;
		if (!inboxUrl) throw error(400, 'Target actor does not expose an inbox');

		activityId = `${getActorId(origin)}/announces/${crypto.randomUUID()}`;
