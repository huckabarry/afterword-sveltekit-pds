<script lang="ts">
	...
	function showPreviousStatus() {
		...
		currentStatusIndex = (currentStatusIndex - 1 + data.statuses.length) % data.statuses.length;
	}
...
<section class="stream-head">
	<div class="stream-head__lede">
		<p>
			I'm Bryan, and this is where I keep the things that would otherwise keep rattling around in my
			head. Some days that means a longer note about cities, housing, design, or family life. Other
			days it's a photo from a walk, a song that sends me back twenty years, a garden update, or a
			small observation that felt worth saving. If you like personal sites, neighborhood wandering,
			and slightly nicer corners of the internet, you're in the right place.
		</p>
	</div>
</section>
	...
	<section class="section-block section-block-status">
		...
		{#if data.statuses.length}
						...
						<article class="status-row" bind:this={activeCardEl}>
							...
							<div class="status-row__body">
								...
								{#if post.quotedPost}
									...
									>
										...
										{#if post.quotedPost.images.length}
											<div
												class="status-row__media status-row__media--quoted {post.quotedPost.images
													.length > 1
								...
								{#if post.images.length}
									<div
										class="status-row__media {post.images.length > 1
											? 'status-row__media--multi'
											: ''}"
									>
			...
			<div class="home-stream-tags">
				...
				<a class="home-updates__more-link" href="/status"
					>Read more status updates <span aria-hidden="true">→</span></a
				>
			</div>
		{:else}
	...
	<section class="section-block section-block-writing">
		...
		{#if data.fieldNotesPosts.length}
			...
			<div class="home-stream-tags">
				...
				<a class="home-updates__more-link" href="/field-notes"
					>Read more field notes <span aria-hidden="true">→</span></a
				>
			</div>
		{:else}
	...
	<section class="section-block section-block-writing">
		...
		{#if data.planningPosts.length}
			...
			<div class="home-stream-tags">
				...
				<a class="home-updates__more-link" href="/planning"
					>Read more planning & urbanism posts <span aria-hidden="true">→</span></a
				>
			</div>
		{:else}
...
<style>
	...
	.section-block-status__card
		.status-row__media:not(.status-row__media--multi)
		.status-row__image
		img {
		height: auto;
		max-height: none;
