			...
			<div class="status-row__actions">
				...
				<div class="status-row__reply-pills">
					...
					>
						<span class="status-row__reply-pill-icon" aria-hidden="true">
							<svg viewBox="0 0 24 24" focusable="false">
								<path d="M18.92 7.64c0-3.18-2.08-4.11-2.08-4.11-1.02-.47-2.77-.67-4.81-.67h-.06c-2.04 0-3.79.2-4.81.67 0 0-2.08.93-2.08 4.11 0 .73-.01 1.6.01 2.51.06 3.05.56 6.06 3.39 6.79 1.31.34 2.44.41 3.47.35 1.87-.11 2.92-.72 2.92-.72l-.06-1.28s-1.34.42-2.84.37c-1.49-.05-3.07-.16-3.31-1.98a3.7 3.7 0 0 1-.03-.49s1.47.36 3.33.44c1.14.05 2.22-.07 3.32-.2 2.11-.25 3.95-1.56 4.18-2.76.36-1.88.37-4.59.37-4.59Zm-2.62 4.69h-1.88V7.76c0-.96-.41-1.45-1.22-1.45-.9 0-1.35.58-1.35 1.72v2.5H10V8.03c0-1.14-.45-1.72-1.35-1.72-.81 0-1.22.49-1.22 1.45v4.57H5.55V7.62c0-.96.25-1.72.76-2.29.52-.57 1.2-.87 2.05-.87.98 0 1.72.37 2.21 1.12l.47.78.47-.78c.49-.75 1.23-1.12 2.21-1.12.85 0 1.53.3 2.05.87.51.57.76 1.33.76 2.29v4.71Z"></path>
							</svg>
						</span>
			...
			{#if data.apReplies.length}
				<section class="status-thread status-thread--fediverse">
					...
					{#each data.apReplies as reply}
						<article class="status-reply">
							<div class="entry-replies__avatar entry-replies__avatar--status">
								{#if reply.avatarUrl}
							...
							<div class="status-reply__body">
								<div class="status-row__meta">
									<div class="status-row__byline">
										<a class="status-row__name status-row__name--link" href={reply.profileUrl} target="_blank" rel="noreferrer">
											{reply.actorName || reply.actorHandle || 'ActivityPub reply'}
										</a>
...
<style>
	...
	.status-row__name--link {
		color: inherit;
		text-decoration: none;
	}

	.status-row__name--link:hover {
		text-decoration: underline;
	}

	.entry-replies__avatar--status {
		width: 2.75rem;
		height: 2.75rem;
	}
