...
<svelte:head>
	<title>{data.post.title} | Now</title>
...
<article class="entry entry--blog h-entry">
	<div class="entry__meta">
		<a href="/now">Now</a>
		<time class="dt-published" datetime={new Date(data.post.publishedAt).toISOString()}
				...
				{#each data.webmentions as mention}
					<li class="entry-webmentions__item">
						<a class="entry-webmentions__link" href={mention.sourceUrl} target="_blank" rel="noreferrer">
							<span class="entry-webmentions__name"
								>{mention.sourceTitle || mention.sourceDomain || mention.sourceUrl}</span
							>
