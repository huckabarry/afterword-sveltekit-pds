...
The public site is built with SvelteKit and runs on Cloudflare Workers, with Cloudflare storage handling some of the heavier lifting behind the scenes. That gives me a live site rather than a static export, which means I can mix archived material with fresher things like Bluesky statuses, check-ins, synced gallery images, and other small present-tense details.
...
Statuses come from Bluesky and my own PDS, and check-ins are now synced in from Swarm into that same personal data layer. I still think of this domain as the center of gravity, but I like that some of the more live pieces can move through a portable identity layer instead of being trapped inside one app. The goal is not to turn the site into a feed, though. It is to let the site breathe a little.
...
Music notes work a little differently. Crucial Tracks and Album Whale entries are archived locally and mirrored into local storage so they can stay part of the site even if the outside services change shape or disappear. The same general logic now applies to the older archive section too: I am trying to bias this site toward keeping what matters close at hand.
